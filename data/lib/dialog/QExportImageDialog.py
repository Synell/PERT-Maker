...
class QExportImageDialog(QDialog):
    def __init__(self, parent = None, lang: dict = {}, bg_color: QUtilsColor = QUtilsColor('#000000'), fg_color = QUtilsColor('#ffffff'), data: QPixmap = None):
        ...
        left_frame.layout_.setSpacing(16)
        ...
        left_frame.layout_.addWidget(QLabel('Background Color'), 0, 0)
        left_frame.layout_.addWidget(self.bg_color, 0, 1)
        left_frame.layout_.setColumnStretch(2, 1)

        right_frame = QGridWidget()
        right_frame.layout_.setSpacing(16)
        ...
        right_frame.layout_.addWidget(QLabel('Foreground Color'), 0, 0)
        right_frame.layout_.addWidget(self.fg_color, 0, 1)
        right_frame.layout_.setColumnStretch(2, 1)

        frame = QGridWidget()
        frame.layout_.setSpacing(16)
        frame.layout_.setContentsMargins(0, 0, 0, 0)
        frame.layout_.addWidget(left_frame, 0, 0)
        frame.layout_.setAlignment(left_frame, Qt.AlignmentFlag.AlignCenter)
        frame.layout_.addWidget(right_frame, 0, 1)
        frame.layout_.setAlignment(right_frame, Qt.AlignmentFlag.AlignCenter)
        ...
        preview_area.layout_.addWidget(self.preview, 0, 0)
        ...
        frame.layout_.setSpacing(16)
        frame.layout_.setContentsMargins(0, 0, 0, 0)
        frame.layout_.addWidget(self.filename, 0, 0)
        frame.layout_.addWidget(right_buttons, 0, 1)
    ...
    def color_changed(self, bg_color: QUtilsColor, fg_color: QUtilsColor) -> QPixmap:
        # self.preview.layout_.itemAt(0).widget().setStyleSheet(f'background-color: {color.ahex};')
        data = self.data.copy()
