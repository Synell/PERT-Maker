...
class Vector2:
    ...
    def scaled(self, scale: object) -> None:
        '''Multiplies this vector by a number or another vector.'''
        if isinstance(scale, Vector2):
            return Vector2(self.x * scale.x, self.y * scale.y)

        elif isinstance(scale, (int, float)):
            return Vector2(self.x * scale, self.y * scale)

        else:
            raise TypeError('Vector2.scaled() takes a Vector2 or a number')
    ...
    def from_angle(angle: float) -> 'Vector2':
        '''Creates a vector with given angle in degrees.'''
        return Vector2(Math.cos(angle * Math.Deg2Rad), Math.sin(angle * Math.Deg2Rad))
