...
class QUpdater(QBaseApplication):
    ...
    def update_title(self):
        self.window.setWindowTitle(self.save_data.get_lang_data('QUpdater.title') + f' | Version: {Info.version} | Build: {Info.build}')
    ...
    def not_implemented(self, text = ''):
        ...
        if text:
            w = QDropDownWidget(text = self.save_data.get_lang_data('QMessageBox.critical.notImplemented.details'), widget = QLabel(text))
        else: w = None
    ...
    def create_widgets(self):
        ...
        self.progress_percent = QLabel(self.save_data.get_lang_data('QUpdater.QLabel.downloading').replace('%s', self.save_data.get_lang_data('QUpdater.QLabel.waiting')))
        self.progress_percent.setProperty('h', 2)
        top_frame.grid_layout.addWidget(self.progress_percent, 0, 0, 1, 2)

        self.progress_eta = QLabel(self.save_data.get_lang_data('QUpdater.QLabel.calculatingRemainingTime'))
        ...
        for text in self.save_data.get_lang_data('QUpdater.QSlidingStackedWidget.texts'):
            w = QGridFrame()
    ...
    def download_speed_changed(self, speed: float):
        self.progress_speed.setText(self.save_data.get_lang_data('QUpdater.QLabel.bytes').replace('%s', self.convert(speed)))

    def download_eta_changed(self, eta: float):
        self.progress_eta.setText(self.save_data.get_lang_data('QUpdater.QLabel.eta').replace('%s', self.convert_time(eta)))
    ...
    def download_progress_changed(self, progress: float):
        ...
        self.progress_percent.setText(self.save_data.get_lang_data('QUpdater.QLabel.downloading').replace('%s', f'{int(progress * 100)} %'))

    def download_done(self):
        self.progress.setValue(50)
        self.progress_percent.setText(self.save_data.get_lang_data('QUpdater.QLabel.downloading').replace('%s', self.save_data.get_lang_data('QUpdater.QLabel.done')))
        self.progress_eta.setText(self.save_data.get_lang_data('QUpdater.QLabel.done'))

    def install_speed_changed(self, speed: float):
        self.progress_speed.setText(self.save_data.get_lang_data('QUpdater.QLabel.items').replace('%s', f'{speed}'))
    ...
    def install_progress_changed(self, progress: float):
        ...
        self.progress_percent.setText(self.save_data.get_lang_data('QUpdater.QLabel.installing').replace('%s', f'{int(progress * 100)} %'))

    def install_done(self):
        self.progress.setValue(100)
        self.progress_percent.setText(self.save_data.get_lang_data('QUpdater.QLabel.done'))
    ...
    def install_failed(self, error: str, exit_code: int):
        QMessageBoxWithWidget(
            app = self,
            title = self.save_data.get_lang_data('QUpdater.QMessageBox.critical.downloadFailed' if exit_code < 2 else 'installFailed.title'),
            text = self.save_data.get_lang_data('QUpdater.QMessageBox.critical.downloadFailed' if exit_code < 2 else 'installFailed.text'),
            informative_text = str(sys.argv[2]),
            icon = QMessageBoxWithWidget.Icon.Critical,
            widget = QDropDownWidget(text = self.save_data.get_lang_data('QMessageBox.critical.notImplemented.details'), widget = QLabel(error))
        ).exec()
    ...
    def convert_time(self, time: timedelta) -> str:
        ...
        if days == -1:
            return self.save_data.get_lang_data('QUpdater.QLabel.undefined')
        elif days > 0:
            return (self.save_data.get_lang_data('QUpdater.QLabel.days') if days > 1 else self.save_data.get_lang_data('QUpdater.QLabel.day')).replace('%s', str(days))
        elif hours > 0:
            return (self.save_data.get_lang_data('QUpdater.QLabel.hours') if hours > 1 else self.save_data.get_lang_data('QUpdater.QLabel.hour')).replace('%s', str(hours))
        elif minutes > 0:
            return (self.save_data.get_lang_data('QUpdater.QLabel.minutes') if minutes > 1 else self.save_data.get_lang_data('QUpdater.QLabel.minute')).replace('%s', str(minutes))
        else:
            return (self.save_data.get_lang_data('QUpdater.QLabel.seconds') if seconds > 1 else self.save_data.get_lang_data('QUpdater.QLabel.second')).replace('%s', str(seconds))
    ...
    def open_app(self):
        if len(sys.argv) > 2:
            try: subprocess.Popen(rf'{sys.argv[2]}', creationflags = subprocess.DETACHED_PROCESS | subprocess.CREATE_NEW_PROCESS_GROUP, cwd = os.getcwd())
                ...
                QMessageBoxWithWidget(
                    app = self,
                    title = self.save_data.get_lang_data('QUpdater.QMessageBox.critical.openAppFailed.title'),
                    text = self.save_data.get_lang_data('QUpdater.QMessageBox.critical.openAppFailed.text'),
                    informative_text = str(sys.argv[2]),
                    icon = QMessageBoxWithWidget.Icon.Critical,
                    widget = QDropDownWidget(text = self.save_data.get_lang_data('QMessageBox.critical.notImplemented.details'), widget = QLabel(str(e)))
                ).exec()
            ...
            QMessageBoxWithWidget(
                app = self,
                title = self.save_data.get_lang_data('QUpdater.QMessageBox.critical.appNotFound.title'),
                text = self.save_data.get_lang_data('QUpdater.QMessageBox.critical.appNotFound.text'),
                icon = QMessageBoxWithWidget.Icon.Critical
            ).exec()
#----------------------------------------------------------------------
